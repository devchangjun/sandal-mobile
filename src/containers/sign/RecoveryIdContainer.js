        ...
        <>
            ...
            <div className={styles['container']}>
                <div className={cx('content','pd-box')}>
                    ...
                    <SignAuthInput inputType={"text"} onClick={onClickRandomAuth} toggle={toggle}initValue={userPhone} buttonTitle={toggle ? "인증번호 재발송" : "인증번호 발송"} onChange={onChangePhone} placeholder={"핸드폰 번호"}/>
                    <div className={cx('auth-btn',{not_view: !toggle})}>
                    <SignAuthInput inputType={"text"} initValue={userAuth} buttonTitle={"인증하기"} onChange={onChangeAuth} onClick={onClickCompareAuth} />
                    </div>
                    <Button title={"확인"} toggle={success} onClick={onClickFindEmail}></Button>
                </div>
